#!/bin/bash
export BUILD_ARCH=$(uname -m |grep aarch64 -q && echo arm64 ; uname -m |grep -q -e amd64 -e amd_64 -e x86_64 && echo amd64 ;  uname -m |grep -q -e 386 -e i686 -e ia32 && echo i386 )
...
##fetch em 
echo   "GET : https://github.com/docker/buildx/releases/download/v${BUILDX_VERSION}/buildx-v${BUILDX_VERSION}.linux-${BUILD_ARCH}" 
...
echo   "GET : https://github.com/regclient/regclient/releases/download/v${REGCTL_VERSION}/${thingy}-linux-${BUILD_ARCH}"
curl -L \
  --output "/$thingy" \
  "https://github.com/regclient/regclient/releases/download/v${REGCTL_VERSION}/${thingy}-linux-${BUILD_ARCH}" && chmod a+x "/$thingy"
done
